...
final class StandardHostValve extends ValveBase {
    ...
    protected void throwable(Request request, Response response, Throwable throwable) {
        ...
        if (errorPage != null) {
            ...
            request.setAttribute(ApplicationFilterFactory.DISPATCHER_TYPE_ATTR, new Integer(ApplicationFilterFactory.ERROR));
            request.setAttribute(Globals.STATUS_CODE_ATTR, new Integer(HttpServletResponse.SC_INTERNAL_SERVER_ERROR));
            request.setAttribute(Globals.ERROR_MESSAGE_ATTR, throwable.getMessage());
            ...
            request.setAttribute(Globals.EXCEPTION_TYPE_ATTR, realError.getClass());
    ...
    protected void status(Request request, Response response) {
        ...
        if (errorPage != null) {
            response.setAppCommitted(false);
            request.setAttribute(Globals.STATUS_CODE_ATTR, new Integer(statusCode));
     ...
     * we successfully utilized the specified error page location, or <code>false</code> if the default error report should be rendered.
     * 
     * @param request
     *            The request being processed
     * @param response
    ...
    protected boolean custom(Request request, Response response, ErrorPage errorPage) {
        ...
        try {
            ...
            Integer statusCodeObj = (Integer) request.getAttribute(Globals.STATUS_CODE_ATTR);
            int statusCode = statusCodeObj.intValue();
            String message = (String) request.getAttribute(Globals.ERROR_MESSAGE_ATTR);
