 ...
 * <p>
 * All subclasses of this abstract base class will include support for a Pipeline object that defines the processing to be performed for each request received
 * by the <code>invoke()</code> method of this class, utilizing the "Chain of Responsibility" design pattern. A subclass should encapsulate its own processing
 * functionality as a <code>Valve</code>, and configure this Valve into the pipeline by calling <code>setBasic()</code>.
 * <p>
 ...
 * <code>ContainerEvent</code> events to listeners who register themselves with <code>addContainerListener()</code>:
 * <table border=1>
 * <tr>
 * <th>Type</th>
 * <th>Data</th>
...
public abstract class ContainerBase implements Container, Lifecycle, Pipeline, MBeanRegistration, Serializable {
    ...
    private static final long serialVersionUID = -3397354854319279507L;

    private static org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory.getLog(ContainerBase.class);
     ...
     * Tomcat.
     */
    protected class PrivilegedAddChild implements PrivilegedAction<Container> {
    ...
    protected HashMap<String/* 容器的名称 */, Container> children = new HashMap<String, Container>();
    ...
    protected String name = null; // The human-readable name of this Container.

    protected Container parent = null;

    protected ClassLoader parentClassLoader = null;

    protected Pipeline pipeline = new StandardPipeline(this); // pipeline对象可以理解为valve对象的容器
    ...
    private Thread thread = null; // The background thread.

    private boolean threadDone = false; // The background thread completion semaphore.
    ...
    public void addChild(Container child) {
        if (System.getSecurityManager() != null) {
            PrivilegedAction<Container> dp = new PrivilegedAddChild(child);
    ...
    public synchronized void start() throws LifecycleException {
        ...
        for (int i = 0; i < children.length; i++) {
            if (children[i] instanceof Lifecycle)
                ((Lifecycle) children[i]).start(); // 启动所有的child
        }
     ...
     * Container. Prior to adding the Valve, the Valve's <code>setContainer</code> method must be called, with this Container
     * as an argument. The method may throw an <code>IllegalArgumentException</code> if this Valve chooses not to
     * be associated with this Container, or <code>IllegalStateException</code> if it is already associated with a different Container.
     * 
     * @param valve
     *            Valve to be added
     * 
