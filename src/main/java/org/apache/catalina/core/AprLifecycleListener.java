...
public class AprLifecycleListener implements LifecycleListener {
    ...
    public void lifecycleEvent(LifecycleEvent event) {
        ...
        if (Lifecycle.INIT_EVENT.equals(event.getType())) {
            ...
            if (patch < RECOMMENDED_PV) {
                if (!log.isDebugEnabled()) {
                    log.info(sm.getString("aprListener.tcnVersion", major + "." + minor + "." + patch,
                            REQUIRED_MAJOR + "." + REQUIRED_MINOR + "." + RECOMMENDED_PV));
                } else {
                    log.debug(sm.getString("aprListener.tcnVersion", major + "." + minor + "." + patch,
                            REQUIRED_MAJOR + "." + REQUIRED_MINOR + "." + RECOMMENDED_PV));
                }
            }
            try {
    ...
    public static synchronized void initializeSSL() throws ClassNotFoundException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {

        if ("off".equalsIgnoreCase(SSLEngine))
            return;
        if (sslInitialized)
            return; // only once per VM
        String methodName = "initialize";
        Class paramTypes[] = new Class[1];
        paramTypes[0] = String.class;
        Object paramValues[] = new Object[1];
