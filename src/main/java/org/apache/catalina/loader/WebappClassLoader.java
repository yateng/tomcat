 ...
 * <p>
 * This class loader is a full reimplementation of the <code>URLClassLoader</code> from the JDK. It is desinged to be fully compatible with a normal
 * <code>URLClassLoader</code>, although its internal behavior may be completely different.
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - This class loader faithfully follows the delegation model recommended in the specification. The system class loader
 * will be queried first, then the local repositories, and only then delegation to the parent class loader will occur. This allows the web application to
 * override any shared class except the classes from J2SE. Special handling is provided from the JAXP XML parser interfaces, the JNDI interfaces, and the
 * classes from the servlet API, which are never loaded from the webapp repository.
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - Due to limitations in Jasper compilation technology, any repository which contains classes from the servlet API will
 * be ignored by the class loader.
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - The class loader generates source URLs which include the full JAR URL when a class is loaded from a JAR file, which
 * allows setting security permission at the class level, even when a class is contained inside a JAR.
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - Local repositories are searched in the order they are added via the initial constructor and/or any subsequent calls to
 * <code>addRepository()</code> or <code>addJar()</code>.
 * <p>
 * <strong>IMPLEMENTATION NOTE</strong> - No check for sealing violations or security is made unless a security manager is present.
 * 
...
public class WebappClassLoader extends URLClassLoader implements Reloader, Lifecycle {
    ...
    protected static org.apache.juli.logging.Log log = org.apache.juli.logging.LogFactory.getLog(WebappClassLoader.class);
     ...
     * earlier versions.
     */
    protected static final String[] triggers = { "javax.servlet.Servlet" // Servlet API
    };
        ...
        for (int i = 0; i < length; i++) {
            try {
                ...
                if (lastModified != lastModifiedDates[i]) {
                    if (log.isDebugEnabled())
                        log.debug("  Resource '" + paths[i] + "' was modified; Date is now: " + new java.util.Date(lastModified) + " Was: "
    ...
    /**
     * Find the specified resource in our local repository, and return a <code>URL</code> refering to it, or <code>null</code> if this resource
     * cannot be found.
     * 
     * @param name
     ...
     * <code>null</code>.
     * <ul>
     * <li>If the <code>delegate</code> property is set to <code>true</code>, call the <code>getResource()</code> method of the parent class loader, if any.</li>
     ...
     * classes in the same manner as <code>loadClass(String, boolean)</code> with <code>false</code> as the second argument.
     * 
     * @param name
     *            Name of the class to be loaded
     * 
     ...
     * <ul>
     * <li>Call <code>findLoadedClass(String)</code> to check if the class has already been loaded. If it has, the same <code>Class</code> object is returned.</li>
     * <li>If the <code>delegate</code> property is set to <code>true</code>, call the <code>loadClass()</code> method of the parent class loader, if any.</li>
     ...
     * </ul>
     * If the class was found using the above steps, and the <code>resolve</code> flag is <code>true</code>, this method will then
                ...
                if (Modifier.isStatic(mods) && Modifier.isFinal(mods)) {
                    ...
                    if (null != value) {
                        ...
                        if (!loadedByThisOrChild(valueClass)) {
                            if (log.isDebugEnabled()) {
                                log.debug("Not setting field " + field.getName() + " to null in object of class " + instance.getClass().getName()
     ...
     * to the resource data. If this resource has not been cached, return <code>null</code>.
     * 
     * @param name
