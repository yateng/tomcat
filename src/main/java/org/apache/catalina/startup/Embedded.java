 ...
 * <li>Set the relevant properties of this object itself. In particular, you will want to establish the default Logger to be used, as well as the default Realm
 * if you are using container-managed security.</li>
 * <li>Call <code>createEngine()</code> to create an Engine object, and then call its property setters as desired.</li>
 * <li>Call <code>createHost()</code> to create at least one virtual Host associated with the newly created Engine, and then call its property setters as
 * desired. After you customize this Host, add it to the corresponding Engine with <code>engine.addChild(host)</code>.</li>
 * <li>Call <code>createContext()</code> to create at least one Context associated with each newly created Host, and then call its property setters as desired.
 * You <strong>SHOULD</strong> create a Context with a pathname equal to a zero-length string, which will be used to process all requests not mapped to some
 * other Context. After you customize this Context, add it to the corresponding Host with <code>host.addChild(context)</code>.</li>
 * <li>Call <code>addEngine()</code> to attach this Engine to the set of defined Engines for this object.</li>
 * <li>Call <code>createConnector()</code> to create at least one TCP/IP connector, and then call its property setters as desired.</li>
 * <li>Call <code>addConnector()</code> to attach this Connector to the set of defined Connectors for this object. The added Connector will use the most
 * recently added Engine to process its received requests.</li>
 * <li>Repeat the above series of steps as often as required (although there will typically be only one Engine instance created).</li>
...
public class Embedded extends StandardService implements Lifecycle {
    ...
    public void setRedirectStreams(boolean redirectStreams) {
        ...
        support.firePropertyChange("redirectStreams", new Boolean(oldRedirectStreams), new Boolean(this.redirectStreams));
     ...
     * 
     * @param address
     *            InetAddress to bind to, or <code>null</code> if the
     ...
     * </pre>
     * 
     * which will also cause the Host to be started if the Engine has already been started. If this is the default (or only) Host you will be defining, you may
     * also tell the Engine to pass all requests not assigned to another virtual host to this one:
     * 
     * <pre>
