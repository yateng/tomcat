 ...
 * 
 * <P>
 * Message headers, as specified in RFC822, include a field name and a field body. Order has no semantic significance, and several fields with the same name may
 * exist. However, most fields do not (and should not) exist more than once in a header.
 * 
 * <P>
 * Many kinds of field body must conform to a specified syntax, including the standard parenthesized comment syntax. This class supports only two simple
 * syntaxes, for dates and integers.
 * 
 * <P>
 * When processing headers, care must be taken to handle the case of multiple same-name fields correctly. The values of such fields are only available as
 * strings. They may be accessed by index (treating the header as an array of fields), or by name (returning an array of string values).
 */
