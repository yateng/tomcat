...
public class NioEndpoint {
    ...
    protected ConcurrentLinkedQueue<NioChannel> nioChannels = new ConcurrentLinkedQueue<NioChannel>() {
        ...
        public boolean offer(NioChannel o) {
            ...
            if (running && (!paused) && (size() < curThreads))
    ...
    protected SSLEngine createSSLEngine() {
        ...
        if (ciphersarr.length > 0)
            engine.setEnabledCipherSuites(ciphersarr);
    ...
    public class Poller implements Runnable {
        ...
        public void addEvent(Runnable event) {
            ...
            // }
            events.offer(event);
            if (wakeupCounter.incrementAndGet() < 3)
                selector.wakeup();
        }
                                    ...
                                    try {
                                        ...
                                        try {
                                            if (fk != null && fk.attachment() != null) {
                                                ...
                                                ka.setError(true); // set to collect this socket immediately
                                                try {
                                                    ka.getChannel().getIOChannel().socket().close();
                                                } catch (Exception ignore) {
                                                }
                                                try {
                                                    ka.getChannel().close();
